...
describe('isExtensionAllowed', () => {
  ...
  describe('Valid Cases', () => {
    Object.entries(validFileTypes).forEach(
    ...
    it('validates files when multiple extensions are allowed', () => {
      ...
      const file = new File([''], `test.${pdf.extension}`, {
        type: pdf.mimeType
      });
      const allowedExtensions = Object.values(validFileTypes).map(
        (t) => t.extension
      );
  ...
  describe('Invalid Cases', () => {
    ...
    it('rejects files with extension not in allowed list', () => {
      ...
      const file = new File([''], `test.${jpg.extension}`, {
        type: jpg.mimeType
      });
  ...
  describe('Edge Cases', () => {
    it('handles files with multiple dots in name', () => {
      const { pdf } = validFileTypes;
      const file = new File([''], `my.test.file.${pdf.extension}`, {
        type: pdf.mimeType
      });
