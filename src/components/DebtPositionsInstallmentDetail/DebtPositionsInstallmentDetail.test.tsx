...
import {
  DebtPositionsInstallmentDetail,
  tmpMockData
} from './DebtPositionsInstallmentDetail';
...
describe('DebtPositionsInstallmentDetail', () => {
  ...
  it('shows payment details for PAID debt', () => {
    (tmpMockData as Record<string, Array<DetailData>>).summaryData = (
      tmpMockData as Record<string, Array<DetailData>>
    ).summaryData.map((item: DetailData) =>
      item.label === 'Stato' ? { ...item, value: 'PAID' } : item
    );
    render(<DebtPositionsInstallmentDetail />);
    expect(
      screen.queryByText('installmentDetailPage.paymentInformation')
    ).toBeInTheDocument();
    expect(
      screen.queryByText('installmentDetailPage.noPaymentMade')
    ).toBeNull();
  });
