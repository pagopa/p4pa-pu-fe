...
describe('DebtPositionDetail Component', () => {
  ...
  it('renders the component with correct title and debt position status chip', () => {
    ...
    const titleElements = screen.getAllByText(mockData.debtPositionTypeOrgDescription);
  ...
  it('renders all payment option types correctly', () => {
    ...
    const downPayments = mockData.paymentOptions.filter(option => 
      option.paymentOptionType === 'DOWN_PAYMENT'
    );
  ...
  it('correctly maps installment data for display', async () => {
    ...
    const allValidIUVs = mockData.paymentOptions
      .flatMap(option => 
        option.installments ? option.installments.filter(
          (inst): inst is InstallmentDTO & { iuv: string } => 
            inst !== undefined && inst.iuv !== undefined && typeof inst.iuv === 'string'
        ) : []
      )
    ...
    const unpaidInstallments = mockData.paymentOptions.flatMap(option => 
      option.installments 
        ? option.installments.filter(installment => 
    ...
    const reportedInstallments = mockData.paymentOptions.flatMap(option => 
      option.installments 
        ? option.installments.filter(installment => 
          installment && installment.status === 'REPORTED'
        ) 
        : []
    );
    ...
    const toSyncInstallments = mockData.paymentOptions.flatMap(option => 
      option.installments 
        ? option.installments.filter(installment => 
          installment && installment.status === 'TO_SYNC'
        ) 
        : []
    );
    ...
    if (toSyncInstallments.length > 0) {
      const toSyncChips = screen.getAllByText((content) => 
        content === 'To Sync' || content === 'Reported'
      );
  ...
  it('correctly displays data for down payment option type', () => {
    ...
    if (downPaymentOptions.length > 0) {
      ...
      if (downPaymentOptions[0].installments && downPaymentOptions[0].installments.length > 0) {
        ...
        if (firstInstallment.remittanceInformation) {
          const infoElements = screen.getAllByText(firstInstallment.remittanceInformation);
    ...
    if (grids.length > 0) {
      ...
      statusChips.forEach(chip => {
        const hasColorClass = Array.from(chip.classList).some(className => 
          ['MuiChip-colorSuccess', 'MuiChip-colorError', 'MuiChip-colorInfo', 'MuiChip-colorDefault'].includes(className)
        );
